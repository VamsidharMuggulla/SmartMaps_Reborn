package com.vehicles;

import javax.ws.rs.Consumes;
import javax.ws.rs.FormParam;
import javax.ws.rs.GET;
import javax.ws.rs.POST;
import javax.ws.rs.Path;
import javax.ws.rs.PathParam;
import javax.ws.rs.Produces;
import javax.ws.rs.QueryParam;
import javax.ws.rs.core.MediaType;
import javax.ws.rs.core.Response;

import org.apache.log4j.Logger;

import com.exceptions.CustomExceptions;
import com.exceptions.ErrorNumbers;
import com.smartmaps.LoginService;

@Path("vehicle/location")
public class VehicleTrackerService {

	
	private static final Logger LOGGER = Logger.getLogger(LoginService.class);
	Response resp=null;
	String x=null;
	
	@Path("/update")
	//@GET
	@POST
	//@Consumes("application/x-www-form-urlencoded")
	@Produces(MediaType.APPLICATION_JSON)	
public Response	trackerService(
		//@QueryParam("trackerid") String vehicleID,   //FOR GET
		//@QueryParam("destination") String destination,
		//@QueryParam("clatitude") String clatitude,
		//@QueryParam("clongitude") String clongitude,
		//@QueryParam("status") String status
		
		@FormParam("trackerID") String vehicleID,          //For POST
		//@FormParam("destination") String destination,
		@FormParam("latitude") String clatitude,
		@FormParam("longitude") String clongitude,
		@FormParam("status") String status
		) {
	
		System.out.println("TEST 1"+vehicleID+clatitude+clongitude+status);
	VehicleTrackerDB vehicleTrackerDB=new VehicleTrackerDB();
	try {
		x=vehicleTrackerDB.getStop(vehicleID,clatitude,clongitude,status);
	} catch (Exception e) {
		// TODO Auto-generated catch block
		 CustomExceptions hasce =  new CustomExceptions().getBundledError(ErrorNumbers.SM_2002,e.getMessage().toString());
      		LOGGER.error("CLASS: UserLogin, METHOD: userService, ERROR :" + e.getMessage(),e);
      		return Response.ok(hasce.getErrorJsonObject(hasce.getErrorCode(),hasce.getErrorMessage())).build();	
		//e.printStackTrace();
	}
    resp=Response.ok(x).build();
	return resp;
		
}
}
